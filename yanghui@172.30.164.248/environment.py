...
# used to modify state (boundig box)
warp = np.array(
    [
        [-1, 0, -1, 0],  # left
        [1, 0, 1, 0],  # right
...
class Env(object):
    def __init__(self, root_dir, step_size):
        ...
        ### Modified: add step_size, each time moves 0.03*H or W
        self.step_size = 0.03
    ...
    def step(self, action):
        ...
        #### modified : add step_size ####
        # self.state += warp[action]
        self.state += warp[action] * self.state * self.step_size
        self.step_count += 1
